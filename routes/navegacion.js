...
module.exports = function(app) {
  ...
  app.get('/activar-cuenta/:cuenta', activarCuenta, function(req, res) {
    res.render('activar-cuenta', { title: 'Activar cuenta' })
  })
}

function activarCuenta (req, res, next) {
  ...
  if(isNaN(req.params.cuenta)) {
    ...
    pool.getConnection(function(err,connection){
      if (err) {
        ...
        connection.query("select * from proyecto.usuarios where id = ?;", [req.params.cuenta],
        function(err,rows){
          if(err) {
            ...
            if (rows.length > 0){
              ...
              connection.query("update proyecto.usuarios set activo = true where id = ?;", [req.params.cuenta],
              function(err,rows){
                if(err) {
                  res.json({"code" : 100, "status" : "Error en la consulta"})
